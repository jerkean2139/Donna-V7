  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="flex items-start justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-text-primary">Cognitive Objects</h1>
          <p className="mt-3 max-w-2xl text-text-secondary">
            Cognitive Objects are the universal intelligence records for Donna V7.
          </p>
        </div>
        <Link className="rounded-lg bg-cyan px-5 py-3 text-bg-base" href="/cognitive-objects/new">
          New object
        </Link>
      </div>
      ...
      <form className="mt-8 flex flex-wrap items-end gap-3">
        ...
        <label>
          ...
          <select
            ...
            className="mt-1 rounded-lg border border-border-default p-2.5 text-sm capitalize"
          >
        ...
        <button type="submit" className="rounded-lg bg-cyan px-4 py-2.5 text-sm text-bg-base">
          Apply
        </button>
        {isFiltering && (
          <Link className="px-2 py-2.5 text-sm text-text-muted hover:text-text-primary" href="/cognitive-objects">
            Clear
          </Link>
        )}
      </form>
        ...
        {objects.length === 0 ? (
          ...
          objects.map((object) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-text-secondary">{object.summary ?? "No summary yet."}</p>
            </Link>
          ))
        )}
      </div>
      ...
      {!isFiltering && (currentPage > 1 || hasNextPage) && (
        <nav aria-label="Pagination" className="mt-8 flex items-center justify-between gap-4">
          {currentPage > 1 ? (
            <Link
              className="rounded-lg border border-border-default px-4 py-2 text-sm hover:bg-bg-surface-2"
