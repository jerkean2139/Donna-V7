...
export default async function NewRelationshipPage({ params }: NewRelationshipPageProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <Link className="text-sm text-text-secondary underline" href={`/cognitive-objects/${object.id}`}>
        ← Back to {object.title}
      </Link>
      <h1 className="mt-4 text-3xl font-bold tracking-tight text-text-primary">Add relationship</h1>
      ...
      {candidates.length === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed border-border-default p-8 text-text-secondary">
